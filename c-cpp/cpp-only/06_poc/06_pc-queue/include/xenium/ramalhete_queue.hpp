 ...
 * \[[Ram16](index.html#ref-ramalhete-2016)\].
 *
 * It is faster and more efficient than the `michael_scott_queue`, but less
 * generic as it can only handle pointers or trivially copyable types that are
 * smaller than a pointer (i.e., `T` must be a raw pointer, a `std::unique_ptr`
 * or a trivially copyable type like std::uint32_t). Note: `std::unique_ptr` are
 * supported for convinience, but custom deleters are not yet supported.
 *
 * A generic version that does not have this limitation is planned for a future
 * version.
 *
 * Supported policies:
 *  * `xenium::policy::reclaimer`<br>
 *    Defines the reclamation scheme to be used for internal nodes.
 * (**required**)
 *  * `xenium::policy::backoff`<br>
 *    Defines the backoff strategy. (*optional*; defaults to
 * `xenium::no_backoff`)
 *  * `xenium::policy::entries_per_node`<br>
 *    Defines the number of entries for each internal node. It is recommended to
 * make this a power of two. (*optional*; defaults to 512)
 *  * `xenium::policy::pop_retries`<br>
 *    Defines the number of iterations to spin on a queue entry while waiting
 * for a pending push operation to finish. (*optional*; defaults to 1000)
 *
 * @tparam T
 * @tparam Policies list of policies to customize the behaviour
 */
template <class T, class... Policies> class ramalhete_queue {
private:
...
public:
  using value_type = T;
  using reclaimer =
      parameter::type_param_t<policy::reclaimer, parameter::nil, Policies...>;
  using backoff =
      parameter::type_param_t<policy::backoff, no_backoff, Policies...>;
  static constexpr unsigned entries_per_node =
      parameter::value_param_t<unsigned, policy::entries_per_node, 512,
                               Policies...>::value;
  static constexpr unsigned pop_retries =
      parameter::value_param_t<unsigned, policy::pop_retries, 1000,
                               Policies...>::value;

  static_assert(entries_per_node > 0,
                "entries_per_node must be greater than zero");
  static_assert(parameter::is_set<reclaimer>::value,
...
private:
  ...
  using marked_value =
      xenium::marked_ptr<std::remove_pointer_t<raw_value_type>, 1>;
  ...
  struct node : reclaimer::template enable_concurrent_ptr<node> {
    // pop_idx and push_idx are incremented by step_size to avoid false sharing,
    // so the actual index has to be calculated modulo entries_per_node
    ...
    explicit node(raw_value_type item)
        : pop_idx{0}, push_idx{step_size}, next{nullptr} {
    ...
    ~node() override {
      for (unsigned i = pop_idx; i < push_idx; i += step_size) {
        traits::delete_value(entries[i % entries_per_node]
...
void ramalhete_queue<T, Policies...>::push(value_type value) {
  ...
  for (;;) {
    ...
    if (idx >= max_idx) {
      ...
      if (next == nullptr) {
        ...
        if (t->next.compare_exchange_strong(expected, new_node,
          ...
          _tail.compare_exchange_strong(expected, new_node,
                                        std::memory_order_release,
                                        std::memory_order_relaxed);
          return;
        }
        ...
        _tail.compare_exchange_strong(expected, next, std::memory_order_release,
                                      std::memory_order_relaxed);
      }
      continue;
    }
    ...
    // (8) - this release-CAS synchronizes-with the acquire-load (14) and the
    // acquire-exchange (15)
    if (t->entries[idx].value.compare_exchange_strong(
            expected, raw_val, std::memory_order_release,
            std::memory_order_relaxed)) {
      traits::release(value);
      return;
    }
...
bool ramalhete_queue<T, Policies...>::try_pop(value_type &result) {
  ...
  for (;;) {
    ...
    // This synchronization is necessary to avoid a situation where we see an
    // up-to-date pop_idx, but an out-of-date push_idx and would (falsly) assume
    ...
    if (idx >= max_idx) {
      ...
      if (_head.compare_exchange_strong(expected, next,
                                        std::memory_order_release,
                                        std::memory_order_relaxed)) {
    ...
    if constexpr (pop_retries > 0) {
      ...
      while (value == nullptr && ++cnt <= pop_retries) {
        ...
        retry_backoff(); // TODO - use a backoff type that can be configured
                         // separately
      }
    }
    ...
    value = h->entries[idx].value.exchange(marked_value(nullptr, 1),
