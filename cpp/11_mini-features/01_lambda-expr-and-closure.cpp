...
void testOneNaiveLambda() {
  ...
  a name: func*/
  auto func = []() { cout << "testOneNaiveLambda(): Hello, World!" << endl; };
  func();
}
...
auto closureWrapper1() {
  int x = 10;
  return [x]() {
    std::cout << "Value in the closure from closureWrapper1(): " << x
              << std::endl;
  };
}
...
function<void(void)> closureWrapper2() {
  ...
  return [&x]() {
    x += 1;
    std::cout << "Value in the closure from closureWrapper2(): " << x
              << std::endl;
  };
}
