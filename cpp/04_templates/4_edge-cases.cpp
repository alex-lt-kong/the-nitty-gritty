...
void partially_supported_templates() {
    ...
    cout << "int first: " << MyIntClass.getFirst() << ", "
         ...
         << "int getProduct():" << MyIntClass.getProduct()
         << endl;

    SimpleClass<string> MyStrClass(
        helper_gen_random_string(5),
        helper_gen_random_string(5)
    );
    cout << "str first: " << MyStrClass.getFirst() << ", "
         << "str second: " << MyStrClass.getSecond() << ", "
         << "str getSum():" << MyStrClass.getSum()
    //   << "str getProduct():" << MyStrClass.getProduct() won't work, as string * string is not defined.
         << endl;         
}
...
void t_must_be_deducible() {
    ...
    // (Well this example may not be too convining as it should be "easy" for the compiler to figure out that
    // people want to use double as the common type)
