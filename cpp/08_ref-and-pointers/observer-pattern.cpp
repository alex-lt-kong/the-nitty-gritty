...
public:
    ...
    void update(const std::string& updateMessage) override {
        std::println("Observer {} received update: {}", id_, updateMessage);
    }
};
...
int main() {
    ...
    // Create three observers that last for the entire duration of main.
    ...
    // Create a dynamically scoped observer.
    {
        auto observer4 = std::make_shared<ConcreteObserver>(4);
        subject.attach(observer4);

        std::println("First Notification (with observers 1, 2, 3, 4):");
        subject.notify("Dummy update value - First Update");
        // observer4 goes out of scope after this block.
    }
