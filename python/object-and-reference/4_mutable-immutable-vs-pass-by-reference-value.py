# In Python, everything is an object. For people from the C world (including me), this may sound odd in the first place.
# But this is what the so-called "object-oriented programming" really should be.
# Python is not a strongly-typed language--when we define a variable, we don't have to (and we actually can't) give it a data type.
# This is consistent with the fact that everything is just an object--what can stop us from assigning an object with a different
# address and making it another "data type"?
# (However, when you think about it, you may start questioning what exactly does "strongly-typed"--for example, is C strongly-typed?
# If we are can to define a pointer in C, we are free to cast the value which the pointer is pointing to. In this sense, C is not that strongly-typed either!)
