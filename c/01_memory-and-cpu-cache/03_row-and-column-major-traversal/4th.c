...
int main() {
  ...
  for (int i = 0; i < sizeof(dim) / sizeof(dim[0]); ++i) {
    ...
    arr_ptr1 = (uint32_t*)malloc(d * d * sizeof(uint32_t));
    arr_ptr2 = (uint32_t*)malloc(d * d * sizeof(uint32_t));
    ...
    printf("%0.9lf,\t%8u,\t", delta0, *(arr_ptr1 + ts.tv_sec % d * d + ts.tv_nsec % d));
    ...
    printf("%12.9lf,\t%9u,\t%0.9lf\n", delta1, *(arr_ptr2 + ts.tv_sec % d * d + ts.tv_nsec % d), delta1 - delta0);
