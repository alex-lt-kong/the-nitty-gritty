...
int main(const int, char *argv[]) {
    ...
    */
    ...
    memcpy(memptr, &my_mutex, sizeof(my_mutex));

    printf("shm address: %p [0..%d]\n", memptr, SHM_SIZE - 1);
    while (!ev_flag) {
        pthread_mutex_t *_mutex = (pthread_mutex_t *) memptr;
        if ((rc = pthread_mutex_trylock(_mutex)) != 0) {
            if (rc == EBUSY)
                continue;
            fprintf(stderr, "pthread_mutex_lock() failed, rc: %d\n", rc);
        }
        char prev_buffer[SHM_SIZE];
        strncpy(prev_buffer, memptr + sizeof(pthread_mutex_t),
                SHM_SIZE - sizeof(pthread_mutex_t) - 1);
        if (strncmp(prev_buffer, buffer, SHM_SIZE - sizeof(pthread_mutex_t)) !=
            0) {
            printf("Shm changed! New content:\n%s\n", prev_buffer);
        }
