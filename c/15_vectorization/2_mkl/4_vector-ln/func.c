...
double mkl_ln(uint64_t arr_size, double *restrict vec_in, double *restrict vec_out)
{
    uint64_t t0, t1;
...
double my_ln(uint64_t arr_size, double *restrict vec_in, double *restrict vec_out)
{
    uint64_t t0, t1;
    t0 = get_timestamp_in_microsec();
